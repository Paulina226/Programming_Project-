...
/*frees the resources used by screen_init*/
void screen_destroy()
{
	if (__data)
...
/*obtains screen data to replace certain characters when needed*/
...
/*frees the resources used by screen_area_init*/
...
/*resets cursor to the default position*/
...
/*moves the cursor or relocates it if it is out of bounds*/
...
/*returns a certain value when the area cursor is out of bounds*/
int screen_area_cursor_is_out_of_bounds(Area* area)
{
	return area->cursor > ACCESS(__data, area->x + area->width, area->y + area->height - 1);
}

/*scrolls the screen display for viewing different parts of the game area*/
void screen_area_scroll_up(Area* area)
{
